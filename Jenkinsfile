                ...
                stage('1') {
                    ...
                    stages {
                        ...
                        stage('Instrumented Unit Tests') {
                            steps {
                                catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                                    sh '''./gradlew -PenableCoverage -PlogcatFile=instrumented_unit_logcat.txt -Pemulator=android27 \
                        ...
                        stage('Testrunner Tests') {
                            steps {
                                catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                                    sh '''./gradlew -PenableCoverage -PlogcatFile=testrunner_logcat.txt -Pemulator=android27 \
                                                startEmulator createCatroidDebugAndroidTestCoverageReport \
                        ...
                        stage('Quarantined Tests') {
                            ...
                            steps {
                                catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                                    sh '''./gradlew -PenableCoverage -PlogcatFile=quarantined_logcat.txt -Pemulator=android27 \
                                            startEmulator createCatroidDebugAndroidTestCoverageReport \
                        ...
                        stage('RTL Tests') {
                            steps {
                                catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                                    sh '''./gradlew -PenableCoverage -PlogcatFile=rtltests_logcat.txt -Pemulator=android27 \
                                            startEmulator createCatroidDebugAndroidTestCoverageReport \
                ...
                stage('2') {
                    ...
                    stages {
                        stage('Pull Request Suite') {
                            steps {
                                catchError(buildResult: 'FAILURE' ,stageResult: 'FAILURE') {
                                    sh '''./gradlew copyAndroidNatives -PenableCoverage -PlogcatFile=pull_request_suite_logcat.txt -Pemulator=android27 \
                                            startEmulator createCatroidDebugAndroidTestCoverageReport \
