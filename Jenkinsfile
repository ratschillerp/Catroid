                ...
                stage('1') {
                    ...
                    stages {
                        ...
                        stage('Instrumented Unit Tests') {
                            steps {
                                catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                                    sh '''./gradlew -PenableCoverage -PlogcatFile=instrumented_unit_logcat.txt -Pemulator=android29 \
                        ...
                        stage('Testrunner Tests') {
                            steps {
                                catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                                    sh '''./gradlew -PenableCoverage -PlogcatFile=testrunner_logcat.txt -Pemulator=android29 \
                                                startEmulator createCatroidDebugAndroidTestCoverageReport \
                        ...
                        stage('Quarantined Tests') {
                            ...
                            steps {
                                catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                                    sh '''./gradlew -PenableCoverage -PlogcatFile=quarantined_logcat.txt -Pemulator=android29 \
                                            startEmulator createCatroidDebugAndroidTestCoverageReport \
                        ...
                        stage('RTL Tests') {
                            steps {
                                catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                                    sh '''./gradlew -PenableCoverage -PlogcatFile=rtltests_logcat.txt -Pemulator=android29 \
                                            startEmulator createCatroidDebugAndroidTestCoverageReport \
